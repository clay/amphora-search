...
function hasPage(uri) {
  return elastic.existsDocument(uri);
}
...
function getPage(uri) {
  return elastic.getDocument(PAGES_INDEX, uri);
}
...
function updatePage(uri, data) {
  ...
  return elastic.update(PAGES_INDEX, uri, data, true, true)
...
function parseUpdateUrl(uriOrUrl) {
  ...
  return clayutils.isPage(uriOrUrl) ? uriOrUrl.split('.html')[0].replace(`${parsed.protocol}//`, '').replace(`:${parsed.port}`, '') : string.split('?')[0];
}
