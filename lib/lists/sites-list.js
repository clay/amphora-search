...
function generateSitesIndexOps(acc, site) {
  // check with amphoraKey, but if the version of amphora we are using doesn't have it,
  // it means we don't have subsite support and are safe to just use slug
  var mediaDir = path.resolve(process.cwd(), site.assetDir, 'media', 'sites', site.amphoraKey || site.slug),
    mediaDirFiles = files.getFiles(mediaDir);

  // if we used amphoraKey to check above, and a subsite doesn't have any files, lets see if parent site does
  if (site.amphoraKey && mediaDirFiles.length == 0 && site.subsite) {
    mediaDir = path.resolve(process.cwd(), site.assetDir, 'media', 'sites', site.slug);
    mediaDirFiles = files.getFiles(mediaDir);
  }

  const icons = _.intersection(acceptedIcons, mediaDirFiles); // https://lodash.com/docs/4.17.2#intersection

  acc.push({
    index: {
      _index: SITES_INDEX,
      _type: '_doc',
      _id: site.amphoraKey || site.slug
    }
  }, {
    name: site.name,
    slug: site.slug,
