...
function generateSitesIndexOps(acc, site) {
  var mediaDir = path.resolve(process.cwd(), site.assetDir, 'media', 'sites', site.amphoraKey),
    mediaDirFiles = files.getFiles(mediaDir);

  // if a subsite doesn't have any files, lets see if parent site does
  if (mediaDirFiles.length == 0 && site.subsite) {
