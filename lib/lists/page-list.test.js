...
describe(filename, () => {
  describe('index', () => {
    test('it sets the pages index', () => {
      expect(lib.index()).toEqual('pages');
    });
  });

  describe('filter', () => {
    test('it returns true if key or uri is page', () => {
      expect(lib.filter({ key: 'example.com/_pages/foo' })).toEqual(true);
      expect(lib.filter({ uri: 'example.com/_pages/foo' })).toEqual(true);
    });

    test('it returns false if key or uri is not page', () => {
      expect(lib.filter({ key: 'example.com/_layouts/foo' })).toEqual(false);
      expect(lib.filter({ uri: 'example.com/_layouts/foo' })).toEqual(false);
    });
  });

  describe('serialize', () => {
    const obj = {
      key: 'example.com/_pages/foo@published',
      value: { foo: 'bar' }
    };

    test('it removes @published from the key or uri', () => {
      expect(lib.serialize(obj)).toEqual({
        key: 'example.com/_pages/foo',
        value: { foo: 'bar' }
      });
