...
describe(filename, () => {
  describe('subscribe', () => {
    ...
    test('it clones ops for save and delete to prevent mutations', () => {
    ...
    test('allows explicit stream ids to be set through { id } option', () => {
      lib.subscribe('save', { id: 'my-consumer' });

      const stream = lib['clay:save']._consumers
        .filter((c) => c.id === 'save:my-consumer')
        .pop();

      expect(stream).toBeTruthy();
      expect(stream.id).toEqual('save:my-consumer');
    });
  });
});
