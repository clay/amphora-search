...
describe(filename, () => {
  ...
  describe('redirectToLogin', () => {
    ...
    test('calls res.redirect', () => {
      ...
      _.set(setup, 'options.sites.getSiteFromPrefix',jest.fn().mockReturnValue({prefix: 'site.com', port: 3001}));
    ...
    test('uses port 80 if one is not defined', () => {
      ...
      _.set(setup, 'options.sites.getSiteFromPrefix',jest.fn().mockReturnValue({prefix: 'site.com'}));
      fn({uri: 'site.com/path/_search'}, res);
      expect(res.redirect).toHaveBeenCalled();
    });
  });
