  ...
  describe(filename, function () {
    ...
    describe('normalizeOpValuesWithMapping', function () {
      ...
      it('normalizes op with string type', function () {
        let op = [ { type: 'put',
            key: 'localhost.dev.nymag.biz/daily/intelligencer/_components/article/instances/civzg5hje000kvurehqsgzcpy',
            value:
      ...
      it('does not invoke a comparitor if the type does not exist', function () {
        let op = [ { type: 'put',
            key: 'localhost/sitename/_components/foo/instances/xyz',
            value:
            { primaryHeadline: 'Blaming Clinton’s Base for Her Loss' } } ],
          result = [ { type: 'put',
            key: 'localhost/sitename/_components/foo/instances/xyz',
    ...
    describe('applyOpFilters', function () {
      ...
      it('rejects if index is not supplied', function () {
        const errMsg = 'Suppliend index is undefined or not a string: undefined';

        return fn(batchOps, undefined, func)
          .catch(function (err) {
            expect(err.message).to.equal(errMsg);
          });
      });

      it('rejects if index is supplied but is not a string', function () {
        const errMsg = 'Suppliend index is undefined or not a string: 123';

        return fn(batchOps, 123, func)
          .catch(function (err) {
            expect(err.message).to.equal(errMsg);
          });
      });
