...
function createAliasIfNone(index, prefix) {
  const indexIfPrefix = createIndexName(index, prefix);

  return module.exports.existsAlias(indexIfPrefix)
    .then(function (exists) {

      console.log('HI', `${helpers.indexWithPrefix(index, prefix)}`);

      if (!exists) {
        // Indices should have a different name than the alias
        // Append '_v1' to newly created indices
        return module.exports.initAlias(`${helpers.indexWithPrefix(index, prefix)}`, indexIfPrefix)
          .then(function (result) {
            log('info', 'Creating Elasticsearch alias', indexIfPrefix, ':', result);
          })
          .catch(function (error) {
            log('error', error);
          });
      } else {
        log('info', 'Elasticsearch alias exists at', indexIfPrefix);
      }
    });
}
...
* @returns {Promise}
*/
function validateIndices(mappings, prefix) {
