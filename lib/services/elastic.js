...
function validateIndices(mappings, settings, prefix) {
  return bluebird.all(Object.keys(mappings).map(index => {
      ...
      .then(exists => {
        if (!exists) {
          ...
          return module.exports.createIndexIfNone(indexWithPrefix, settings[index])
            ...
            .catch(error => {
              console.log(`womp`, aliasWithPrefix);
              log('error', error)
            });
        } else {
          log('debug', `Elasticsearch alias exists at ${indexWithPrefix}`);
    ...
    .catch(error => {
      console.log(`'asdfasdf'`, aliasWithPrefix);
...
function scrollStream(opts) {
  return ElasticsearchScrollStream(module.exports.client, opts);
}
...
module.exports.setElasticsearchScrollStream = fake => ElasticsearchScrollStream = fake;
