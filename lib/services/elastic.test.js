...
describe(_.startCase(filename), function () {
  ...
  describe('putSettings', function () {
    ...
    it('calls the `putSettings` method exposed on the client', function () {
      client.indices.putSettings.returns(Promise.resolve(true));
      return fn('pages', {})
        .then(function () {
          sinon.assert.calledOnce(client.indices.putSettings);
        });
    });

    it('putSettings rejects and returns an Error if failed', function () {
      client.indices.putSettings.returns(Promise.reject(new Error('error message')));
      return fn('pages', {})
        .catch(function (error) {
          expect(error).to.be.an.instanceof(Error);
        });
  ...
  describe('scrollStream', function () {
    ...
    it ('returns scroll stream constructed with client and specified opts', function () {
      const mockElasticSearchScrollStream = sandbox.stub();
      let result;

      mockElasticSearchScrollStream.returns({foo: 'bar'});
      lib.setElasticsearchScrollStream(mockElasticSearchScrollStream);
      result = fn({baz: 'zar'});
      expect(mockElasticSearchScrollStream.calledOnce).to.be.true;
