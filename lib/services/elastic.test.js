...
describe(filename, () => {
  ...
  describe('createAliasIfNone', () => {
    ...
    it('calls existsAlias successfully', () => {
    ...
    it('calls initAlias successfully', () => {
    ...
    it('calls initAlias successfully and catches if rejected', () => {
      lib.existsAlias = jest.fn().mockResolvedValue(false);
  ...
  describe('batch', () => {
    ...
    it('calls bulk method successfully', () => {
      client.bulk.mockResolvedValue();
      return fn([])
        .then(() => expect(client.bulk).toHaveBeenCalled());
    });

    it('bulk rejects and returns an Error if failed', () => {
  ...
  describe('putSettings', () => {
    ...
    it('calls the `putSettings` method exposed on the client', () => {
      client.indices.putSettings.mockResolvedValue(true);
      return fn('pages', {})
        .then(() => expect(client.indices.putSettings).toHaveBeenCalled());
    });

    it('putSettings rejects and returns an Error if failed', () => {
      client.indices.putSettings.mockResolvedValue(new Error('error message'));
      return fn('pages', {})
  ...
  describe('scrollStream', () => {
    const fn = lib.scrollStream;

    it ('returns scroll stream constructed with client and specified opts', () => {
      const mockElasticSearchScrollStream = jest.fn();
      let result;

      mockElasticSearchScrollStream.mockReturnValue({foo: 'bar'});
      lib.setElasticsearchScrollStream(mockElasticSearchScrollStream);
      result = fn({baz: 'zar'});

      expect(mockElasticSearchScrollStream).toHaveBeenCalled();
      // expect(mockElasticSearchScrollStream.getCall(0).args[0]).to.eql(lib.client);
      // expect(mockElasticSearchScrollStream.getCall(0).args[1]).to.eql({baz: 'zar'});
      // expect(mockElasticSearchScrollStream.getCall(0).args[1]).to.eql({baz: 'zar'});
