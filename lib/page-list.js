...
function markPagePublished(page) {
  ...
  return existsPage(page)
    ...
    .map(function ({ _id, _source }) {
      ...
      _source.firstPublishTime = _source.firstPublishTime || new Date();
      _source.url = customUrl || url;

      return { _id, _source };
    })
    .errors(err => {
      log('error', `Could not update page document status to 'published': ${err.message}`, {
        stack: err.stack
      });
    });
}
...
function getPageById(id) {
  if (!id) throw new Error('Page id is falsy, a string value is required');
