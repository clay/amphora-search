...
function parseUpdateUrl(string) {
  ...
  return utils.isPage(string) ? string.split('.html')[0].replace(`${parsedUrl.protocol}//`, '').replace(`:${parsedUrl.port}`, '') : string;
}

/**
 * [constructFindQuery description]
 * @param  {[type]} reqObj [description]
 * @return {[type]}        [description]
 */
function constructFindQuery(reqObj) {
  var queryUrl = parseUpdateUrl(reqObj.url); // Need the protocol to strip that from the url. also, it's assuming HTML
...
function findPageAndUpdate(reqObj) {
  ...
  return elastic.query('pages', constructFindQuery(reqObj), 'general')
    .then(updatePageListEntry(reqObj.value));
}
