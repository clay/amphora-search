...
/**
 * TODO: CLEAN THIS ALL UP AND TEST FOR ERRORS
 * @param  {[type]} reqObj [description]
...
function findPageAndUpdate(reqObj) {
  ...
  parsedUrl = url.parse(_.clone(reqObj.url), true, true);

  // Need the protocol to strip that from the url. also, it's assuming HTML
  queryUrl = reqObj.url.indexOf('/pages/') > -1 ? reqObj.url.split('.html')[0].replace(`${parsedUrl.protocol}//`, '').replace(`:${parsedUrl.port}`, '') : reqObj.url;

  query = {
    query: {
			"multi_match" : {
		      query: queryUrl,
		      fields: [ 'url', '_id', 'mainContent' ]
		    }
		}
  };

  return elastic.query('pages', query, 'general')
    .then(resp => {
      var hit = _.get(resp, 'hits.hits[0]', undefined);

      if (hit) {
        console.log('Trying to update page document at: ', queryUrl, 'with data: ', reqObj.value);
        return module.exports.updatePageEntry(hit._id, reqObj.value);
      }
